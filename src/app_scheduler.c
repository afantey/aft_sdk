                             ...
                             app_sched_event_handler_t handler)
{
    ...
    if (event_data_size <= m_queue_event_size)
    {
        ...
        {
            err_code = SDK_E_MEMORY;
        }
    }
    else
    {
        err_code = SDK_E_INVALID;
    }
 ...
 *
 * @return      SDK_OK if new event, SDK_E_INVALID if event queue is empty.
 */
...
{
    uint32_t err_code = SDK_E_INVALID;
...
void app_sched_execute(void)
{
    ...
    while ((app_sched_event_get(&p_event_data, &event_data_size, &event_handler) == SDK_OK))
    {
