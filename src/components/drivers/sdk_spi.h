...
struct rt_spi_ops
{
    sdk_err_t (*configure)(struct rt_spi_device *device, struct rt_spi_configuration *configuration);
...
/* register a SPI bus */
sdk_err_t rt_spi_bus_register(struct rt_spi_bus       *bus,
                             const char              *name,
                             const struct rt_spi_ops *ops);

/* attach a device on SPI bus */
sdk_err_t rt_spi_bus_attach_device(struct rt_spi_device *device,
                                  const char           *name,
                                  const char           *bus_name,
                                  void                 *user_data);
 ...
 */
sdk_err_t rt_spi_take_bus(struct rt_spi_device *device);
 ...
 */
sdk_err_t rt_spi_release_bus(struct rt_spi_device *device);
 ...
 */
sdk_err_t rt_spi_take(struct rt_spi_device *device);
 ...
 */
sdk_err_t rt_spi_release(struct rt_spi_device *device);

/* set configuration on SPI device */
sdk_err_t rt_spi_configure(struct rt_spi_device        *device,
                          struct rt_spi_configuration *cfg);

/* send data then receive data from SPI device */
sdk_err_t rt_spi_send_then_recv(struct rt_spi_device *device,
                               const void           *send_buf,
                               size_t             send_length,
                               void                 *recv_buf,
                               size_t             recv_length);

sdk_err_t rt_spi_send_then_send(struct rt_spi_device *device,
 ...
 */
size_t rt_spi_transfer(struct rt_spi_device *device,
                          const void           *send_buf,
                          void                 *recv_buf,
                          size_t             length);
...
rt_inline size_t rt_spi_recv(struct rt_spi_device *device,
                                void                 *recv_buf,
                                size_t             length)
{
...
rt_inline size_t rt_spi_send(struct rt_spi_device *device,
                                const void           *send_buf,
                                size_t             length)
{
 ...
 * @return the actual length of transmitted.
 */
sdk_err_t rt_qspi_configure(struct rt_qspi_device *device, struct rt_qspi_configuration *cfg);
 ...
 * @return the actual length of transmitted.
 */
sdk_err_t rt_qspi_bus_register(struct rt_spi_bus *bus, const char *name, const struct rt_spi_ops *ops);
 ...
 */
sdk_err_t rt_qspi_send_then_recv(struct rt_qspi_device *device, const void *send_buf, size_t send_length,void *recv_buf, size_t recv_length);
 ...
 * @return the status of transmit.
 */
sdk_err_t rt_qspi_send(struct rt_qspi_device *device, const void *send_buf, size_t length);
