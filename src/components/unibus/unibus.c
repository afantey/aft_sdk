...
unibus_t *unibus_find_dest_dev(uint8_t dest_id, unibus_t *unibus_dev_table[], int dev_num)
{
    ...
    {
        for(int j = 0; j < UNIBUS_MAX_SUB_ID; j++)
        {
            if(unibus_dev_table[i]->sub_id[j] == dest_id) 
            {
                return unibus_dev_table[i];
            }
        }
    }
    return NULL;
 ...
 * @return int 
 */
static int unibus_body_get(unibus_t *ctx, uint8_t cmd_fun, uint16_t cmd_index)
{
 ...
 * 
 * ****************/
static int unibus_read_reg_send(unibus_t *ctx, uint8_t src_id, uint8_t dest_id, uint16_t index, int read_reg_num)
{
    uint8_t msg[UNIBUS_MAX_MSG_LEN] = {0};
    int msg_len;
...
PT_THREAD(unibus_read_reg_poll(unibus_t *ctx, struct pt *pt, struct swtimer *read_reg_timer, uint32_t response_timeout, uint8_t src_id, uint8_t dest_id, uint16_t index, int read_reg_num, uint16_t *reg, int *reg_num))
{
    ...
    unibus_read_reg_send(ctx, src_id, dest_id, index, read_reg_num);
    swtimer_set(read_reg_timer, response_timeout);
    PT_YIELD_UNTIL(pt, ((bodyget_ret = unibus_body_get(ctx, CMD_CMAP_ACK_RD, index)) != 0) || ((swtimer_ret = swtimer_expired(read_reg_timer)) != 0));
    if(swtimer_ret == 1)
    {
        // LOG_D("read reg timeout: src[0x%02x] dest[0x%02x] index[0x%04x]\n", src_id, dest_id, index);
        *reg_num = UNI_ERR_MSG_TIMEOUT;
    }
...
static int unibus_write_reg_send(unibus_t *ctx, uint8_t src_id, uint8_t dest_id, uint16_t index, int write_reg_num, uint16_t *reg)
{
    uint8_t msg[UNIBUS_MAX_MSG_LEN] = {0};
    int msg_len;
    uni_data_16_t data16 = {0};
...
PT_THREAD(unibus_write_reg_poll(unibus_t *ctx, struct pt *pt, struct swtimer *write_reg_timer, uint32_t response_timeout, uint8_t src_id, uint8_t dest_id, uint16_t index, int write_reg_num, uint16_t *reg, int *ret_write))
{
    ...
    unibus_write_reg_send(ctx, src_id, dest_id, index, write_reg_num, reg);
    swtimer_set(write_reg_timer, response_timeout);
    PT_YIELD_UNTIL(pt, ((bodyget_ret = unibus_body_get(ctx, CMD_CMAP_ACK_WR, index)) != 0) || ((swtimer_ret = swtimer_expired(write_reg_timer)) != 0));
    if(swtimer_ret == 1)
    {
        LOG_D("write reg timeout: src[0x%02x] dest[0x%02x] index[0x%04x]\n", src_id, dest_id, index);
        *ret_write = UNI_ERR_MSG_TIMEOUT;
    }
    else if(bodyget_ret == 1)
    {
 ...
 * 
 * ****************/
int unibus_send_read_ack(unibus_t *ctx, msg_meta_t *p_msg_meta, int read_len, uint16_t *reg_tab)
{
    uint8_t msg[UNIBUS_MAX_MSG_LEN] = {0};
    int msg_len;
...
static PT_THREAD(unibus_receive_msg(unibus_t *ctx, msg_meta_t *p_msg_meta, uint8_t *msg, int *ret_rl, int wait))
{
    int length_to_read;
    uint32_t rl = 0;
    int ret = 0;
...
PT_THREAD(unibus_recv_poll(unibus_t *ctx, int host_id))
{
    ...
    PT_SPAWN(&ctx->pt_recv_poll, &ctx->pt_receive_msg, unibus_receive_msg(ctx, &msg_meta, ctx->msg, &rl, 0));
    if (rl > 0)
    {
        // LOG_D("[%02x->%02x] body_len %d, cmd %02x, index %02x\r\n",
        ...
        if (ctx->parser == NULL)
        {
            ret = UNI_ERR_NULL_PARSER;
            LOG_E("unibus_recv_poll return UNI_ERR_NULL_PARSER\n");
        }

        if (msg_meta.dest_id == host_id) // 目标是本机地址
        {
            for (int i = 0; i < UNIBUS_CMD_MAX_OP; i++)
            {
                    ...
                    && ctx->parser->own_cmd_ops[i].cmd_handler != NULL)
                {
                    ...
                    if(ret != 0)
                    {
                        LOG_E("unibus_recv_poll return %d\n", ret);
                    }
                }
            }
        }
