                         ...
                         _Bool          flag)
{
    ...
    if (flag)
    {
        n = snprintf(string_for_signature, sizeof(string_for_signature), "%u\n%s\nproducts/%s\n%s", et, METHOD, res, ver);
    }
    else
    {
        n = snprintf(string_for_signature,
    ...
    if (flag)
    {
        n = snprintf(authorization_buf,
                 authorization_buf_len,
                 "version=%s&res=products%%2F%s&et=%u&method=%s&sign=%s",
                 ver,
                 res,
                 et,
                 METHOD,
    ...
    else
    {
        n = snprintf(authorization_buf,
                 authorization_buf_len,
                 "version=%s&res=products%%2F%s%%2Fdevices%%2F%s&et=%u&method=%s&sign=%s",
                 ver,
                 res,
                 dev_name,
                 et,
