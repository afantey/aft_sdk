...
//////////////////////////需要不同平台对接的函数///////////////////////////////////////////////////
// int apbus_serial_read(void *pctx, uint8_t *rbuff, int len, int wait);
// int apbus_serial_write(void *pctx, uint8_t *pbuff, int len);
///////////////////////////////////////////////////////////////////////////////////////
 ...
 * @return int 
 */
static int apbus_serial_write(void *pctx, const uint8_t *pbuff, int len)
{
    apbus_t *ctx;
    ctx = (apbus_t *)pctx;
...
struct apbus_serial_read_pt_data
{
    uint32_t sum;
    int timeout_ms;
    int remaining_len;
...
static struct apbus_serial_read_pt_data serial_pt_data = {
    .sum = 0,
    .timeout_ms = 0,
    .remaining_len = 0,
    .step = 0,
...
#define LOOP_DELAY 1
static PT_THREAD(apbus_serial_read(void *pctx, uint8_t *rbuff, int len, uint32_t *psum, int wait))
{
    apbus_t *ctx;
    ctx = (apbus_t *)pctx;
    uint32_t rl = 0;
    struct apbus_serial_read_pt_data *pt_data = (struct apbus_serial_read_pt_data *)ctx->backend->pt_data; 

    PT_BEGIN(&ctx->pt_backend_read);

    memset(pt_data, 0, sizeof(struct apbus_serial_read_pt_data));
 ...
 * @return int 
 */
static int apbus_serial_make_msg(void *pctx, msg_meta_t *p_msg_meta, uint8_t *msg, int *p_msg_len)
{
    int offset = 0;
 ...
 * @return int 
 */
static int apbus_serial_check_integrity(void *pctx, uint8_t *msg, int msg_len, msg_meta_t *p_msg_meta)
{
    apbus_t *ctx = (apbus_t *)pctx;
    uint16_t body_len = 0;
    uni_data_16_t checksum = {0};

    if (msg[0] != 0xBC || msg[1] != 0xCB)
    {
        return APBUS_ERR_PROTO_BROKEN;
    }
...
#if 0
static int _apbus_serial_flush(apbus_t *ctx)
{
    uint8_t buff[128];
    uint32_t l = 0;
    uint32_t sum = 0;
...
static int apbus_serial_connect(void *pctx)
{
    apbus_t *ctx;
    ctx = (apbus_t *)pctx;
    ...
    sdk_uart_control((sdk_uart_t *)ctx->port, SDK_CONTROL_UART_INT_IDLE_ENABLE, NULL);
    //sdk_uart_write((sdk_uart_t *)ctx->port, (uint8_t *)"apbus!\n", strlen("apbus!\n"));
    return APBUS_ERR_NO;
}

static void apbus_serial_close(void *pctx)
{
    apbus_t *ctx;
    ctx = (apbus_t *)pctx;
...
const apbus_backend_t backend_uart = {
    .header_length    =  _APBUS_SERIAL_HEADER_LENGTH  ,
    .checksum_length  =  _APBUS_SERIAL_CHECKSUM_LENGTH,
    .max_data_length  =  _APBUS_SERIAL_MAX_DATA_LENGTH,
    .pt_data          = &serial_pt_data                ,
    .read             =  apbus_serial_read            ,
    .write            =  apbus_serial_write           ,
    .make_send_msg    =  apbus_serial_make_msg        ,
    .check_integrity  =  apbus_serial_check_integrity ,
    .receive          =  NULL                          ,
    .prepare_response =  NULL                          ,
    .connect          =  apbus_serial_connect         ,
    .close            =  apbus_serial_close           ,
};
